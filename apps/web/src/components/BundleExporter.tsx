...
import { copyText } from '@iconforge/shared';
import { apiUrl, assetUrl, API_BASE_URL } from '@/lib/api-client';
...
export function BundleExporter({ open, onClose }: Props) {
  ...
  const tgzUrl = assetUrl(`/api/packages/${framework}.tgz?style=${encodeURIComponent(style)}`);
  const zipUrl = assetUrl(`/api/packages/${framework}?style=${encodeURIComponent(style)}`);
  const spriteUrl = assetUrl(`/sprite/${encodeURIComponent(style)}`);
  const sampleSvgUrl = assetUrl(`/svg/laptop-${encodeURIComponent(style)}`);
  void API_BASE_URL; // hint to bundler not to drop the import in case the value is unset
  ...
  const handleDownload = async (format: 'zip' | 'tgz') => {
    ...
    try {
      const url =
        format === 'tgz'
          ? apiUrl(`/api/packages/${framework}.tgz?style=${encodeURIComponent(style)}`)
          : apiUrl(`/api/packages/${framework}?style=${encodeURIComponent(style)}`);
